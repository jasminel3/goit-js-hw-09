...
const storeFormFieldsInLocalStorage = (event)=>{
    ...
    if (email !== "" || message !== "") {
        ...
        localStorage.setItem(FEEDBACK_FORM_STATE_KEY, serializedFeedbackStorageData);
    }
};
...
const submitfeedbackForm = (event)=>{
    ...
    if (email === "" || message === "") {
        alert("TOATE C\xc2MPURILE TREBUIE COMPLETATE.");
        feedbackForm.reset();
    }
...
loadDataFromStorage();
(0, _lodashThrottleDefault.default)(storeFormFieldsInLocalStorage, 500);
// FUNCTION CALL
feedbackForm.addEventListener("input", storeFormFieldsInLocalStorage);
feedbackForm.addEventListener("submit", submitfeedbackForm); //test
