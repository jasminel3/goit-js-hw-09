...
const numberOfNotifications = document.querySelector('.form input[name="amount"]');
...
form.addEventListener("submit", (e)=>{
    ...
    for(let i = 0; i < contorValue; i++){
        ...
        createPromise(i, currentDelay).then(()=>{
            (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${i + 1} in ${currentDelay} ms`);
        }).catch(()=>{
            (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${i + 1} in ${currentDelay} ms`);
        });
